...
namespace SYSBA.Catalogos
{
    public partial class fCargo : Form
    {
        ...
        private void CargaDatos()
        {
            try
            {
                this.cargoTableAdapter.Fill(this.sYSBADataSet.Cargo, null, this.txtBuscar.Text.Trim(), true);
                this.dgvDatos.DataSource = this.sYSBADataSet.Cargo;
        ...
        private void Eliminar()
        {
            try
            {
                this.cargoTableAdapter.Delete(this.Id);
                this.CargaDatos();
            }
            catch (Exception ex)
            {
        ...
        private void Guardar()
        {
            try
            {
                ...
                {
                    case "I":
                        this.cargoTableAdapter.Insert(this.nombreTextBox.Text.Trim(), true);
                        this.Mensaje = "El registro de guardó correctamente.";
                        this.Titulo = "Guardar";
                        this.Icono = MessageBoxIcon.Information;
                        break;
                    case "M":
                        this.cargoTableAdapter.Update(this.nombreTextBox.Text.Trim(), true, this.Id);
        ...
        private void dgvDatos_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                if (this.dgvDatos.Rows.Count == 0)
                    return;
                this.Id = int.Parse(this.dgvDatos.CurrentRow.Cells["IdCargo"].Value.ToString());
