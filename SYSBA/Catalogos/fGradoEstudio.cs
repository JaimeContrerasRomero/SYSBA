...
{
    public partial class fGradoEstudio : Form
    {
        public fGradoEstudio()
        {
            InitializeComponent();
        }

        private void fGradoEstudio_Load(object sender, EventArgs e)
        {
            this.CargaDatos();
        }
        ...
        private void CargaDatos()
        {
            try
            {
                this.gradoEstudioTableAdapter.Fill(this.sYSBADataSet.GradoEstudio, null, this.txtBuscar.Text.Trim(), true);
                this.dgvDatos.DataSource = this.sYSBADataSet.GradoEstudio;
        ...
        private void Eliminar()
        {
            try
            {
                this.gradoEstudioTableAdapter.Delete(this.Id);
                this.CargaDatos();
            }
            catch (Exception ex)
            {
        ...
        private void Guardar()
        {
            try
            {
                ...
                {
                    case "I":
                        this.gradoEstudioTableAdapter.Insert(this.nombreTextBox.Text.Trim(), true);
                        this.Mensaje = "El registro de guardó correctamente.";
                        this.Titulo = "Guardar";
                        this.Icono = MessageBoxIcon.Information;
                        break;
                    case "M":
                        this.gradoEstudioTableAdapter.Update(this.nombreTextBox.Text.Trim(), true, this.Id);
        ...
        private void dgvDatos_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                if (this.dgvDatos.Rows.Count == 0)
                    return;
                this.Id = int.Parse(this.dgvDatos.CurrentRow.Cells["IdGradoEstudio"].Value.ToString());
